...
public class TransformShapeSynthesizer(
    ...
    private fun analyzeStmt(stmt: io.github.ehlyzov.branchline.Stmt): ShapeState = when (stmt) {
        is LetStmt -> {
    ...
    private fun analyzeProperty(prop: io.github.ehlyzov.branchline.Property): ShapeState = when (prop) {
    ...
    private fun analyzeOutputProperty(prop: io.github.ehlyzov.branchline.Property): ShapeState = when (prop) {
        is LiteralProperty -> analyzeExpr(prop.value)
        is ComputedProperty -> mergeSequential(analyzeExpr(prop.keyExpr), analyzeExpr(prop.value))
    }
    ...
    private fun analyzeLambdaExpr(expr: io.github.ehlyzov.branchline.LambdaExpr): ShapeState {
        val lambdaScope = scopes.last().toMutableSet().apply { addAll(expr.params) }
        return when (val body = expr.body) {
            is io.github.ehlyzov.branchline.ExprBody -> analyzeExprWithScope(body.expr, lambdaScope)
            is io.github.ehlyzov.branchline.BlockBody -> analyzeBlock(body.block, lambdaScope)
        }
    }
